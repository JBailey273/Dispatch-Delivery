...
export default function DispatchDropDetailPage() {
  ...
  return <main>
    ...
    {drop && <>
      ...
      <button onClick={async()=>{try { const res = await api(`/dispatch/drops/${id}/send-reschedule-sms`, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message, admin_override:adminOverride})}); alert(`Queued at ${res.sent_at}`); setError(''); } catch (err) { const apiErr = err as ApiError; setError(apiErr.message || 'Failed to send reschedule text.'); } finally { await load(); }}}>Send reschedule text</button>
    </>}
  </main>;
}
