...
const sendSchedulingLink = async () => {
  ...
  const saveNotes = async () => {
    ...
    try {
      ...
      setNotesMsg((err as ApiError).message || 'Failed to save notes');
    } finally {
  ...
  const [addLoadSaving, setAddLoadSaving] = useState(false);
  const [addLoadMsg, setAddLoadMsg] = useState('');
  const [addLoadSuccess, setAddLoadSuccess] = useState(false);

  const submitAddLoad = async () => {
    if (!addLoadBulkGroup || addLoadQty < 1) return;
    setAddLoadSaving(true);
    setAddLoadMsg('');
    try {
      await api(`/drops/${dropId}/add-load`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          bulk_group: addLoadBulkGroup,
          qty: addLoadQty,
          payment_note: addLoadPaymentNote.trim() || null,
        }),
      });
      setAddLoadSuccess(true);
      setShowAddLoad(false);
      setAddLoadQty(1);
      setAddLoadPaymentNote('');
      await refreshDetail();
      setTimeout(() => setAddLoadSuccess(false), 3000);
    } catch (err) {
      setAddLoadMsg((err as ApiError).message || 'Failed to add material');
    } finally {
      setAddLoadSaving(false);
    }
  };
          ...
          <div className="so-body">
            ...
            <div className="so-card">
              ...
              {dropDetail.customer_id ? (
                <Link href={`/customer-search?search=${encodeURIComponent(dropDetail.customer_name)}`} className="so-info-name so-info-name-link">
              ...
              {dropDetail.loads.map(load => {
                ...
                return (
                  <div key={load.id} className={`so-load${isEditing ? ' editing' : ''}${isException ? ' so-load-exception' : ''}`}>
                    ...
                    {/* Split off panel */}
                    {splitLoadId === load.id && (
                      <div style={{ margin: '0 16px 12px', padding: '12px 14px', background: 'var(--blue-50,#eff6ff)', border: '1px solid var(--blue-200,#bfdbfe)', borderRadius: 'var(--radius-md)' }}>
                        {splitResult ? (
            ...
            <div className="so-card">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                <div className="so-card-label" style={{ marginBottom: 0 }}>Notes</div>
                {!editingNotes && (
                  <button className="so-add-load-toggle" onClick={startEditNotes}>✏️ Edit</button>
                )}
              </div>
              {editingNotes ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
                  ...
                  />
                  {notesMsg && (
                    <div style={{ fontSize: 12, color: 'var(--red-600)' }}>{notesMsg}</div>
                  )}
                  <div style={{ display: 'flex', gap: 8 }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--gray-400)', fontStyle: 'italic' }}>No notes on this order.</div>
              )}
            </div>
...
const panelStyles = `
  ...
  /* Add load / edit toggle */
  .so-add-load-toggle { padding: 3px 10px; border: 1px solid var(--border); border-radius: var(--radius-md); background: var(--surface); font-family: var(--font-heading); font-size: 11px; font-weight: 600; color: var(--gray-600); cursor: pointer; transition: all 0.15s; white-space: nowrap; }
